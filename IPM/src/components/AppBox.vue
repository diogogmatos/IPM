<template>
  ...
  >
    <div class="flex space-x-3">
      ...
      <InfoTag v-if="status === 'realizado'" color="green"
        >Realizado ·
        {{
          date &&
          `${padNumber(date.getDate(), 2)}/${padNumber(date.getMonth(), 2)} · ${padNumber(date.getHours(), 2)}:${padNumber(date.getMinutes(), 2)}h`
        }}</InfoTag
      >
      ...
      >
      <InfoTag v-else-if="status === 'cancelado'" color="red"
        >Cancelado ·
        {{
          date &&
          `${padNumber(date.getDate(), 2)}/${padNumber(date.getMonth(), 2)} · ${padNumber(date.getHours(), 2)}:${padNumber(date.getMinutes(), 2)}h`
